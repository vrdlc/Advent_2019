      ...
      if (dir == "R") {
        ...
        coordX = coordX + Number(wire1[i])
          for (r = prevCoordX; r < coordX; r++) {
            wire1Array.push({x : r, y : prevCoordY});
          }
        prevCoordX = coordX;
...
//This writes all array entries into the console
    // for (w = 0; w < wireMatchArray.length; w++) {
    //   console.log("Match Array: " + wireMatchArray[w].x + ", " + wireMatchArray[w].y);
    // }

//Finally, we calculate the Manhattan distance of each matching coordDiff
    for (z = 0; z < wireMatchArray.length; z++) {
      x = Math.abs(wireMatchArray[z].x);
      y = Math.abs(wireMatchArray[z].y);
      sum = x + y;
      console.log("X " + x + typeof(x) + ", Y " + y + typeof(y));
      manDistArray.push(sum);
      console.log("Man Dist Array: " + manDistArray[z].x + ", " + manDistArray[z].y);
    }
    if (manDistArray.sort((a,b) => a-b)[0] == 0) {
      console.log("Shortest Distance: " + manDistArray[1])
    } else {
      console.log("Shortest Distance: " + manDistArray[0])
    }
}
